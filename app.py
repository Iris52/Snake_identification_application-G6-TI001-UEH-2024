...
df = pd.read_csv('D:/Documents/testdata/Csv/test.csv')
df_train = pd.read_csv('D:/Documents/testdata/Csv/train.csv')
...
models = {
    "Binomial": {
        "EfficientNetV2B3": load_model('D:/Documents/my_backend_new/my_backend/Binomial/EfficientNetV2B3.keras'),
    },
}
...
def save_search_history(binomial_label, predictions, additional_info):
    # Chuyển đổi các giá trị trong additional_info sang kiểu dữ liệu chuẩn
    sanitized_additional_info = {
        key: int(value) if isinstance(value, (np.integer, np.int64)) else value 
...
def save_search():
    ...
    save_search_history(binomial_label, predictions, additional_info)
